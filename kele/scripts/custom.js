$(document).ready(function () {      
    ...
    function init_template(){
        ...
        $('.deploy-menu').click(function(){
            var menu_height = $('.header-menu').height() + 5
            var top_offset = $('.header-menu').position().top;
            
            if (top_offset > 0){
                $('.header-menu').css({
                    "transform": "translateY(" + menu_height * (-1) + "px)",
                    "-webkit-transform": "translateY(" + menu_height * (-1) + "px)",
                    "-moz-transform": "translateY(" + menu_height * (-1) + "px)",
                    "-o-transform": "translateY(" + menu_height * (-1) + "px)",
                    "-ms-transform": "translateY(" + menu_height * (-1) + "px)"
                });
                $(this).find('.line-1').removeClass('active-line-1');
                $(this).find('.line-2').removeClass('active-line-2');
                $(this).find('.line-3').removeClass('active-line-3');
            } else {
                $('.header-menu').css({
                    "transform": "translateY(" + (-1) + "px)",
                    "-webkit-transform": "translateY(" + (-1) + "px)",
                    "-moz-transform": "translateY(" + (-1) + "px)",
                    "-o-transform": "translateY(" + (-1) + "px)",
                    "-ms-transform": "translateY(" + (-1) + "px)"
                });   
                $(this).find('.line-1').addClass('active-line-1');
                $(this).find('.line-2').addClass('active-line-2');
                $(this).find('.line-3').addClass('active-line-3');
            }
            return false;
        });
        ...
        $('.close-menu').click(function(){
            ...
            $('.deploy-menu').find('.line-1').removeClass('active-line-1');
            $('.deploy-menu').find('.line-2').removeClass('active-line-2');
            $('.deploy-menu').find('.line-3').removeClass('active-line-3');
        });
        
        //Submenu Open
        $('a[data-submenu]').click(function(event) {
            $('.header-menu').find('.ion-ios-plus-empty').removeClass('rotate-plus');
            var sub_get = $(this).data('submenu');
            var sub_height = $('#'+sub_get).height();
            var sub_item_height = 45
            var sub_item_numbers = $('#'+sub_get).children().length;
            
            if (sub_height == 0){
                $(this).find('.ion-ios-plus-empty').addClass('rotate-plus');
                $(this).addClass('active-item');
                $('.submenu').css({"height":"0px"});
                $('#'+sub_get).css({
                    "height": sub_item_height * sub_item_numbers +"px"
                })
            } else {
                $(this).find('.ion-ios-plus-empty').removeClass('rotate-plus');
        ...
        if($('.submenu').hasClass('active-submenu')){
            var id_item = $('.active-submenu').attr('id')
            var item_target = $('a[data-submenu="'+id_item+'"]')
            item_target.find('i:last-child').addClass('rotate-plus');
        }
        
        //Submenu Active
        if( $('.submenu').hasClass('active-submenu')){
            var submenu_items = $('.active-submenu').children().length;
            var sub_item_height = 45
            var sub_total = submenu_items * sub_item_height
            $('.active-submenu').css({
                "height": sub_total 
            }) 
        }
        ...
        function home_sizes(){
            var landing_width = $(window).width();
            var landing_height = $(window).height();
            var landing_header = $('.landing-header').height() + 40;
            var landing_adjusted_height = landing_height-landing_header
            var line_height = landing_adjusted_height/3

            $('.landing-content-2').css({
               "height": landing_adjusted_height
            });

            $('.landing-content-2 a').css({
                "width": (landing_width/3),
                "height": landing_adjusted_height/3,
                "float":"left",
                "line-height":(line_height-20)+"px"
            });
